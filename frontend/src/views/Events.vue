...
<<<<<<< HEAD
        <div class="event-details">
          ...
          <p><strong>Min/Max:</strong> {{ event.min_participants }} - {{ event.max_participants }}</p>
          <p><strong>Game:</strong> {{ event.game_name }} (ID: {{ event.game_id }})</p>
        ...
        <button
          v-else
          @click="leaveEvent(event.event_id)"
          class="leave-button"
        >
          Leave
        </button>
...
const joinEvent = async (eventId) => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/join_event/${eventId}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: user_id.value, user_mail: user_mail.value }),
    });
...
const fetchData = async () => {
  try {
    ...
    if (!resEvents.ok || !resUpcoming.ok) throw new Error("Failed to fetch events");
...
const fetchJoinedEvents = async () => {
  ...
  try {
    const res = await fetch(`http://localhost:3000/user_events?user_id=${user_id.value}&user_mail=${user_mail.value}`);
